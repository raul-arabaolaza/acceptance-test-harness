...
public class PluginManager extends ContainerPageObject {
    ...
    public boolean installPlugins(final PluginSpec... specs) throws UnableToResolveDependencies, IOException {
        ...
        if (uploadPlugins) {
            ...
            // JENKINS-50790 It seems that this page takes too much time to load due to some to be optimized code in core
            try {
                driver.manage().timeouts().pageLoadTimeout(time.seconds(120), TimeUnit.MILLISECONDS);
                visit("available");
            } finally {
